...
class Cryocon22C:
    ...
    def set_max_power_verified(self, value: float, loop: int = 1) -> float:
        ...
        for cand in candidates:
            self.write(f"LOOP {loop}:MAXPWR {cand}", wait_time=0.05 if self.mock else 0.3)
            time.sleep(0.05 if self.mock else 0.2)
    ...
    def arm_safe_ramp(
        ...
        try:
            ...
            self.write("LOOP 1:TYPE PID", wait_time=0.05 if self.mock else 0.3)
            time.sleep(0.05 if self.mock else 0.3)
            self.write(f"LOOP 1:SETPT {live_temp:.3f}", wait_time=0.05 if self.mock else 0.5)
            time.sleep(0.05 if self.mock else 0.5)

            # Step 2: Configure loop gains and limits
            self.write(f"LOOP 1:RANGE {range_val}", wait_time=0.05 if self.mock else 0.8)
            time.sleep(0.05 if self.mock else 0.5)  # Mechanical relay switch
            self.set_max_power_verified(maxpwr)
            self.write(f"LOOP 1:PGAIN {p:.1f}", wait_time=0.02 if self.mock else 0.2)
            self.write(f"LOOP 1:IGAIN {i:.1f}", wait_time=0.02 if self.mock else 0.2)
            self.write(f"LOOP 1:DGAIN {d:.1f}", wait_time=0.02 if self.mock else 0.2)
            self.write(f"LOOP 1:RATE {rate:.2f}", wait_time=0.02 if self.mock else 0.2)

            # Step 3: Engage control while setpoint is parked at live reading
            self.write("CONTROL", wait_time=0.05 if self.mock else 0.3)
            time.sleep(0.1 if self.mock else 2.5)  # Allow loop to balance at hold power
            ...
            self.write("LOOP 1:TYPE RAMPP", wait_time=0.05 if self.mock else 0.4)
            time.sleep(0.05 if self.mock else 0.4)
            self.write(f"LOOP 1:SETPT {target_temp:.3f}", wait_time=0.05 if self.mock else 0.4)
            ...
            for attempt in range(1, 4):
                ...
                self.write("LOOP 1:TYPE RAMPP", wait_time=0.05 if self.mock else 0.3)
                time.sleep(0.05 if self.mock else 0.3)
                self.write(f"LOOP 1:SETPT {target_temp:.3f}", wait_time=0.05 if self.mock else 0.5)
